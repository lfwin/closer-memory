...
import load_cifar10_binary_label
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"


(X, Y), (X_test, Y_test) = load_cifar10_binary_label.load_data()
#X, Y = shuffle(X, Y)
            ...
            for i, v in enumerate(conv_params):
                if i == 0:
                    if n_input_plane != n_output_plane:
                        ...
                        convs = tflearn.activation(convs, 'relu')
...
def train_model(depth, k, angle, lr, tensorboard_dir, run_id, res_dir, epochs=60):
...
#     parser.add_argument("runned_num", type=int)
    ...
    run_id = "WRN_16_1_CIFAR10_relu"
    print(res_dir)
    res_dir = tensorboard_dir+run_id+"/model.tfl"
    angle = random.randint(0, 360)

    train_model(depth=args.depth, k=args.k, angle=angle, lr=args.lr, tensorboard_dir=tensorboard_dir, run_id=run_id, res_dir=res_dir)
